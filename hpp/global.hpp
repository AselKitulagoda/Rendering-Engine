...
#define G 9.8f
#define MTLPATH "assets/materials.mtl"
#define OBJPATH "assets/logo.obj"
#define TEXPATH "assets/texture.ppm"
#define MTL_CORNELL "assets/cornell-box.mtl"
#define OBJ_CORNELL "assets/bump-cornell-box.obj"
#define OBJ_SPHERE "assets/sphere.obj"
...
vector<vec3> bumpNormals = loadBumpMap("assets/bump.ppm");

// Loading Triangles
vector<ModelTriangle> cornellTriangles = combineTriangles(readGround(SCALE_CORNELL,"assets/ground.obj"),readCornellBox(SCALE_CORNELL));
vector<ModelTriangle> sphereTriangles = readSphere(SCALE_SPHERE);
vector<ModelTriangle> bumpTriangles = readBumpWall(SCALE_CORNELL, "assets/bumpwall.obj");
vector<ModelTriangle> allTriangles = combineTriangles(combineTriangles(combineTriangles(cornellTriangles, bumpTriangles), readObj(SCALE_FACTOR, "assets/logo.obj")), sphereTriangles);

// Loading triangles into objects
vector<Object> allObjects;

// Vector to store vertex normals
vector<pair<ModelTriangle, vector<vec3>>> triangleVertexNormals;

// Initialising the lights
vec3 lightSource = vec3(-0.0315915, 1.20455, -0.3108);
...
vector<uint32_t> pixelColours = loadImage("assets/texture.ppm");
vector<uint32_t> checkcols = loadCheckImage("assets/chessNEW.ppm");
int texWidth;
int texHeight;

int filenum = 0;
string filepath = "outputs/" + std::to_string(filenum) + ".ppm";
